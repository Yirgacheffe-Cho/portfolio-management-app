...
4.  **시장 환경 기반의 미래 전략 제안 (자산 비율 조정 근거)**: 현재의 금리 추이, 환율 변동, 주요 주식 시장 흐름, 그리고 **최신 세계 정세**를 종합적으로 반영하여, 향후 포트폴리오를 위한 **구체적인 자산군별 액션 플랜(매수/매도/유지 권고)을 제안**해 주십시오. **각 제안의 근거는 반드시 관련 최신 뉴스(뉴스 제목, 날짜, 출처 명시)에 기반**해야 합니다.
...
**✍️ 답변 지침**:
* 당신의 모든 분석과 제안은 **최신 뉴스를 통해 분석된 상황**을 반드시 반영하며, **각 제안의 핵심 근거가 되는 뉴스(뉴스 제목, 날짜, 출처)**를 명확히 명시해야 합니다.
* **미래에 대한 예측 및 제안은 오직 현재까지 수집된 뉴스, 과거 데이터, 그리고 시장 동향 분석에 기반합니다.** 미래에 발생할 가상의 뉴스 이벤트를 가정하거나 추측하여 제시하지 않습니다.
* **만약 특정 뉴스 정보를 찾을 수 없거나, 제시하는 근거가 당신의 추론/예측에 기반한 것이라면, 해당 내용을 'AI 추측: [내용]' 또는 '시장 분석 (AI 추측): [내용]'과 같이 명확히 표기**하여 정보의 성격을 명확히 해 주십시오.
